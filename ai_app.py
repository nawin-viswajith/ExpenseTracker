...
# ─────── Read OAuth credentials from secrets ───────
GOOGLE_CLIENT_ID     = st.secrets["google"]["client_id"]
GOOGLE_CLIENT_SECRET = st.secrets["google"]["client_secret"]
# If you stored redirect_uri in secrets, pull it too; otherwise, hard-code below.
REDIRECT_URI         = st.secrets["google"]["redirect_uri"]

# Where to keep each user’s separate DB
...
# 1) Check if Google just redirected back with a “?code=…” parameter
...
if "code" in query_params:
    ...
    try:
        ...
        google_id = userinfo["id"]  # (‘sub’ if using v3)
...
# 2) If still not logged in, show the “Login with Google” button
...
with tabs[0]:
    ...
    if not df.empty:
        ...
        if df.groupby(df["date"].dt.to_period("M"))["amount"].sum().mean() > 0 and df["amount"].max() > df["amount"].mean() * 3:
        ...
        if avg_monthly > 0 and ratio > 0.3:
            est_savings = ratio * avg_monthly * 0.25
            from babel.numbers import format_currency
            st.success(f"Tip: You could save around {format_currency(est_savings, 'INR', locale='en_IN')} monthly by reducing luxury spending.")
            st.info("You've had some unusually high expenses. Make sure they were planned or essential.")
...
with tabs[2]:
    ...
    if not df.empty:
        ...
        category_total["formatted"] = category_total["amount"].apply(
        ...
        pie_fig.update_traces(
            textinfo="percent+label", hovertemplate="%{label}<br>Total: %{customdata[0]}<extra></extra>"
        )
