...
if st.session_state.page == "auth":
    ...
    with st.sidebar:
        ...
        st.markdown("### Predicted Budget for Next Month")
        forecast_df = df.copy()
        forecast_df['date'] = pd.to_datetime(forecast_df['date'])
        forecast_df['month'] = forecast_df['date'].dt.to_period('M')
        month_totals = forecast_df.groupby('month')['amount'].sum().reset_index()
        month_totals['month'] = month_totals['month'].dt.to_timestamp()

        from sklearn.linear_model import LinearRegression
        import numpy as np

        if len(month_totals) >= 3:
            month_totals['month_num'] = np.arange(len(month_totals))
            X = month_totals[['month_num']]
            y = month_totals['amount']
            model = LinearRegression()
            model.fit(X, y)
            next_month = np.array([[len(month_totals)]])
            predicted = model.predict(next_month)[0]
            st.success(f"Estimated next month budget: {format_currency(predicted, 'INR', locale='en_IN')}")
    ...
    with tabs[0]:
        ...
        if not df.empty:
            ...
            if avg_monthly > 0 and ratio > 0.3:
                est_savings = ratio * avg_monthly * 0.25  # save 25% of non-essential
                st.success(f"Tip: You could save around {format_currency(est_savings, 'INR', locale='en_IN')} monthly by reducing luxury spends by 25%.")
                st.info("You've had some unusually high expenses. Make sure they were planned or essential.")
        df = get_expenses(st.session_state.user_id)
        if not df.empty:
